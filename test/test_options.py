...
from pymodm_motor.connection import DEFAULT_CONNECTION_ALIAS
...
class MotorMongoOptionsTestCase:
    ...
    def test_collection_default(self):
        class MyModel(MotorMongoModel):
            pass

        self.assertEqual(
            'my_model',
            MyModel._mongometa.collection.name)
...
class AsyncIOMotorMongoOptionsTestCase(MotorMongoOptionsTestCase,
                                       AsyncIOMotorODMTestCase):
    pass


@unittest.skipUnless(TORNADO_TEST, 'Tornado or Motor not installed')
class TornadoMotorMongoOptionsTestCase(MotorMongoOptionsTestCase,
