...
class MotorFieldsTestCase:

    @unittest_run_loop
    async def test_field_dbname(self):
        self.assertEqual('firstName', Person.first_name.mongo_name)
        await Person(email='han@site.com', first_name='Han').save()
        self.assertEqual(
            'Han',
    ...
    def test_rename_primary_key(self):
        ...
        with self.assertRaisesRegex(ValueError, msg):
            class RenamedPrimaryKey(MotorMongoModel):
                renamed_pk = fields.CharField(mongo_name='foo',
                                              primary_key=True)

    @unittest_run_loop
    async def test_pk_alias(self):
        await Person('han.solo+wookie@milleniumfalcon.net',
                     'Han', 'Solo').save()
    ...
    @unittest_run_loop
    async def test_implicit_id(self):
        class NoExplicitId(MotorMongoModel):
    ...
    @unittest_run_loop
    async def test_set_empty(self):
        inst = await Simple('a string with more than zero characters').save()
    ...
    @unittest_run_loop
    async def test_save_blank_value(self):
        inst = await Simple('a string with more than zero characters').save()
        inst.name = ''
        await inst.save()
    ...
    def test_simple_required(self):
        class SimpleRequired(MotorMongoModel):
    ...
    def test_default(self):
        class SimpleDefault(MotorMongoModel):
...
class AsyncIOMotorFieldsTestCase(MotorFieldsTestCase,
                                 AsyncIOMotorODMTestCase):
    pass


@unittest.skipUnless(TORNADO_TEST, 'Tornado or Motor not installed')
class TornadoMotorFieldsTestCase(MotorFieldsTestCase,
